...
export default class DhiegoLevel7 extends BaseLevel {
  constructor() {
    super("dhiegolevel7", true);
  }

  preload() {
    this.load.tilemapCSV("dhiegomap7", "src/scenes/screens/Levels/DhiegoLevel/dhiego_level_7.csv");
    this.load.tilemapCSV("dhiegobg7", "src/scenes/screens/Levels/DhiegoLevel/dhiego_level_v2_bg.csv");
  }
  ...
  create() {
    ...
    this.enemies_list7 = [];
    [264, 197, 28, 35].forEach((pos, index) => {
      this.enemies_list7.push(this.makeEnemy(pos, Bee));
    });
    this.enemies_list7_dog = [];
    [353, 272, 116, 106].forEach((pos, index) => {
      this.enemies_list7_dog.push(this.makeEnemy(pos, Dog));
    });
    ...
    zone.on("enterzone", () => this.scene.start("dhiegolevel8"));
  }
  ...
  update() {
    ...
    this.enemies_list7_dog?.forEach((enemy) => {
